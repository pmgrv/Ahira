				...
				<div class="row">
					...
					<div class="sun clearfix">
						...
						<div id="left-block" class="col-sm-12 text-center">
						     <a href="#" id="registerbutton" data-toggle="modal" data-target="#myModal" class="btn  btn-success">Register Patient</a>
						</div>
		...
		<div class="modal-content">
		  ...
		  <div class="modal-body">
			<form role="form" id="registerform">
		      ...
		      <div class="form-group">
				<label>DOB:</label>
				<input type="date" class="form-control" placeholder = "@28-12-1991" id="dob_patient" required="required">
			  </div> <br/>	
		      <div class="form-group">
				<label>Address:</label>
				<input type="text" class="form-control" placeholder = "@Dewalgaon" id="add_at_patient" required="required">
			  </div> <br/>  	
		      <div class="form-group">
				<label>Pin Code:</label>
				<input type="text" class="form-control" placeholder = "@441901" id="add_pin_code_patient" required="required">
			  </div> <br/>   
				<label>Gender:
		      <div class="form-group">
				<input type="radio" name="gender_patient" id="gender_patient" value="male">Male
				<input type="radio" name="gender_patient" id="gender_patient" value="female">Female
			  </div> 
			  </label> <br/>  
		...
		$(document).ready(function(){
		    $(document).on('click','#submitData',function(){
			  ...
			  var json_data = {
			    ...
			    "phoneNo":$("#phNo").val(),
			    "aadhar":$("#aadhar").val(),
			    "dob_patient":$("#dob_patient").val(),
			    "add_pin_code_patient":$("#add_pin_code_patient").val(),
			    "gender_patient":$("input[id=gender_patient]:checked").val(),
			    "add_at_patient":$("#add_at_patient").val()
			  };
			  $.ajax({
			  type:"POST",
			...
			$(document).on('click','#searchHospitalNow',function(){
			  var json_data = {
			    "searchPatientText":$("#searchPatientText").val() 
			  };
			  $.ajax({
			  type:"POST",
			  url:"PHP/search_hospital_availibility.php",
			  data: {"DATA": JSON.stringify(json_data)},
			  beforeSend:function(data){
			    // alert(JSON.stringify(json_data));
			  },
			  success:function(data){
			  	...
			  	if(data['status']=='failure'){
				    ...
				    window.location.replace("searched_result.php?searchPatientText="+data['data']);
			    }
